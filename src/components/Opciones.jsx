...
const Opciones = ({ datos, label, categoria, nombre }) => {
  ...
  return (
    <>
      <label className={OptStyle.label} htmlFor={categoria}>
        Seleccione {categoria == "propiedad" ? "el tipo de" : "su"} {label}
      </label>
      <select
        className={OptStyle.options}
        name={categoria}
        id={categoria}
        defaultValue={0}
        onChange={(e) =>
          setElementos({
            ...elementos,
            [categoria]: isNaN(parseFloat(e.target.value))
              ? 0
